...
description: 服务异常诊断。Pod/服务相关异常时触发，包括：起不来、启动失败、一直重启、CrashLoop、Pending、OOMKilled、部署失败、镜像拉取失败、发布失败、容器异常、pod 挂了、服务挂了、网络不通、无法访问。自动拉取 Pod 状态和事件，分路径定位根因，无需手动操作，需要运维介入时自动给出相应的 oncall 工单创建命令。
---
...
3. **修复建议与行动 (Action)：**
   ...
   - **需运维处理的问题：** 告知用户底层异常情况，并**提供完整的 Oncall 工单创建命令供用户复制**（话术示例：“诊断为底层网络安全组拦截，请复制并发送以下命令联系运维处理：`/oncall eks_ec2 payment-service [test] 网络连通性异常 - 需 SRE 介入排查网络链路`”）。
...
2. **节点亲和性 (Node Affinity)：**
   - 报错 `didn't match node affinity/selector` 由**运维**维护。在回复中给出工单创建命令让用户联系运维处理，**严禁引导研发去改发布配置**。
...
判断为底层基础设施/运维问题时，**必须在最终回复中给出格式完整的 `/oncall <值班模块> <问题描述>` 命令，供用户复制执行**。

请根据排查得到的实际 `serviceName` 和 `env` 填充占位符，**严禁在最终给用户的输出中保留 `{serviceName}` 或 `{env}` 等原始大括号占位符**（如未获取到相关变量，则需先向用户追问确认，再给出对应的命令）：

| 根因场景 | 推荐给用户的命令格式 |
|---|---|
| 节点资源不足 / IP 池耗尽 / 只有 Insufficient | `/oncall eks_ec2 {serviceName} [{env}] Pod Pending - 节点资源不足，需扩容` |
| node affinity/selector 不匹配 (含 NotTriggerScaleUp) | `/oncall eks_ec2 {serviceName} [{env}] Pod Pending - node affinity 不匹配，需运维检查节点组标签` |
| SGP 安全组不一致（5000034） | `/oncall eks_ec2 {serviceName} [{env}] SGP 不一致，需更新 SecurityGroupPolicy` |
| ECR 镜像拉取权限（403） | `/oncall eks_ec2 {serviceName} [{env}] 镜像拉取失败 - 节点组 IAM Role 缺少 ECR 权限` |
| 宿主机故障 / NodeNotReady | `/oncall eks_ec2 {serviceName} [{env}] Pod 卡 Terminating - 宿主机故障，需强制删除` |
| Finalizer 未清理 | `/oncall eks_ec2 {serviceName} [{env}] Pod 卡 Terminating - Finalizer 未清理` |
| metrics 未采集 | `/oncall eks_ec2 {serviceName} [{env}] metrics 未采集，需运维排查 Prometheus 采集配置` |
...
4. **禁用工具：** 严禁调用任何以 `cmdb_` 和 `cmdb-`开头的工具，且**严禁调用 `analyze`、`svc`、`execute` 工具**。排障过程只能依赖 Kubernetes 原生或文中指定的其他允许的排障 MCP 工具。
